...
export const BucketListApp = () => {
  ...
  const [bucketLists, setBucketLists] = useState<BucketList[]>(
    JSON.parse(
      localStorage.getItem("listStorage") || JSON.stringify(hardCodedValues)
    )
  );
  ...
  const removeBucketListFunction = (id: string) => {
    ...
    localStorage.setItem(
      "listStorage",
      JSON.stringify(bucketLists.filter((bucketList) => bucketList.id !== id))
    );
  };

  const addBucketListFunction = (
    newBucketListName: string,
    newBucketListPlace: string
  ) => {
    const newId = uuidv4();
    setBucketLists([
      ...bucketLists,
      new BucketList(newId, newBucketListName, newBucketListPlace, false),
    ]);
    localStorage.setItem(
      "listStorage",
      JSON.stringify([
  ...
  const checkboxFunction = (id: string) => {
    setBucketLists((prevBucketLists) => {
      const updatedBucketLists = prevBucketLists.map((bucketlist) =>
        bucketlist.id === id
          ? { ...bucketlist, isDone: !bucketlist.isDone }
          : bucketlist
      );
  ...
  const handleSort = () => {
    setBucketLists(
      [...bucketLists].sort((a, b) => a.place.localeCompare(b.place))
    );
    localStorage.setItem(
      "listStorage",
      JSON.stringify(
